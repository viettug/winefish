...
static gboolean doc_view_key_release_lcb( GtkWidget *widget, GdkEventKey *kevent, Tdocument *doc ) {
	func_complete_show(widget, kevent, doc->bfwin, FUNC_FROM_OTHER);
...
Tdocument *doc_new( Tbfwin* bfwin, gboolean delay_activate )
{
	...
	*/
	...
	/* g_object_set (G_OBJECT (newdoc->spell_tag), "underline_color", "red", NULL); */
	newdoc->spell_mark_i = gtk_text_buffer_create_mark(newdoc->buffer, NULL, &iter, FALSE);
	newdoc->spell_mark_ii = gtk_text_buffer_create_mark(newdoc->buffer, NULL, &iter, FALSE);
